  ...
  module MethodHelpers
    ...
    def lookup(*args)
      ...
      if method_name.is_a?(Regexp)
        MethodHelpers.lookup_matching_methods(target, method_name)
      else
        MethodHelpers.lookup_single_method(target, method_name)
      end
      nil
    end
    alias ls lookup
    ...
    class << self
      ...
      def lookup_matching_methods(base, method_filter)
        each_target_level(base) do |target, type|
          ...
          puts " #{type.to_s.titleize} Methods ".center(ConsoleHelpers.width - 10, '-'), "\n"
          puts DisplayString.new(methods).output(:methods), "\n"
        end
      end
      ...
      def print_method_location(method)
        print '  '
        ConsoleHelpers.print_file_link(*method.source_location, 'unknown')
      end
    end
  end
...
Object.include ConsoleHelpers::MethodHelpers
# Needed because the delegate classes will try to pass everything it can up to the object, leading to it being
# skipped in the list, since self points to the base object, not the delegate class instance.
Delegator.include(ConsoleHelpers::MethodHelpers) if defined?(Delegator)
